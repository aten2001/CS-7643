...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = dnext_h*(1-next_h**2)  # dtanh = 1 -tanh^2
    dx = dtanh.dot(Wx.T)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(x.shape[1]):
        h[:,t,:],n_cache = rnn_step_forward(x[:,t,:],prev_h,Wx,Wh,b)
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):    
        dx_, dprev_ht, dWx_, dWh_, db_ = rnn_step_backward(dh[:, i, :] + dprev_ht, cache.pop())    
        dx[:, i, :] = dx_    
        dh0 = dprev_ht    
        dWx += dWx_    
        dWh += dWh_    
